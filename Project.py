...
def transactions():
    ...
    data = st.session_state['db'].execute(f'select transaction_id, symbol, shares, price, shares * price, transacted from '{transaction}' where user_id = {st.session_state["user"]} order by transacted desc;')
    data = data.fetchall()
    if not data:
