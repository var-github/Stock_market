...
def portfolio():
    ...
    data = st.session_state['db'].execute(f"select symbol, sum(shares), sum(shares*price) from '{transaction}' where user_id = {st.session_state['user']} group by symbol having sum(shares) != 0;")
    data = data.fetchall()
    st.text(data)
    if not data:
...
:orange[Learn Strategies]: Access resources and tools designed to help you understand market trends, analyze stocks, and refine your investment approach.\n
:orange[Educational Value]: Offering a platform that not only entertains but also educates, helping users build confidence and competence in stock market investments.\n
:orange[User-Centric Design]: Focusing on creating an intuitive and engaging experience for all users, from beginners to seasoned investors.\n
\n
##### Connect with Me\n
I’m always open to feedback, questions, or collaborations. Feel free to reach out to me at [LinkedIn](https://www.linkedin.com/in/varun-ravichandran-1a778a304).""")
